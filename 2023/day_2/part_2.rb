...
games_data = DATA.map { |game|
  ...
  result["rounds"] = {}
  rounds.each_with_index { |round, index|
    result["rounds"][index] = {}
    round.split(",").each { |cube_group|
...
min_cube_counts = games_data.map { |game_data|
  game_data["rounds"].each_with_object({}) { |round, result|
...
puts min_cube_counts.map { |cube_count|
  cube_count.reduce(1) { |power, cube|
    power * cube[1]
  }
}.sum
