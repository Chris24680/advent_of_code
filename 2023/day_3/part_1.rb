engine_layout = DATA.each_line.to_a.map(&:strip).map { |line|
  line.chomp.chars
}
...
number_indexes.each_with_index { |row, y|
  row.each_with_index { |character, x|
    next if scanned.include?("#{x} #{y}")
    if character.is_a?(String) && /\d/.match?(character)
      number = [character]
...
engine_layout.each_with_index { |row, y|
  row.each_with_index { |character, x|
    if !/\d/.match?(character) && !/\./.match?(character)
      (-1..1).each { |y_offset|
        (-1..1).each { |x_offset|
          next if x + x_offset < 0 || y + y_offset < 0 || x + x_offset >= row.length || y + y_offset >= engine_layout.length
